...
describe('ChoiceButton', () => {
  it('renders with default state correctly', () => {
    ...
    const { getByText } = render(
      <ThemeProvider>
        <ChoiceButton text="Test Option" onPress={onPressMock} state="default" />
      </ThemeProvider>
    );
  ...
  it('does not trigger onPress when disabled', () => {
    ...
    const { getByText } = render(
      <ThemeProvider>
        <ChoiceButton text="Test Option" onPress={onPressMock} state="default" disabled={true} />
      </ThemeProvider>
    );
  ...
  it('renders different states', () => {
    ...
    states.forEach(state => {
      const { getByText } = render(
        <ThemeProvider>
          <ChoiceButton text={`Option ${state}`} onPress={onPressMock} state={state} />
        </ThemeProvider>
      );
  ...
  it('passes index prop for staggered animations', () => {
    ...
    const { getByText } = render(
      <ThemeProvider>
        <ChoiceButton text="Test Option" onPress={onPressMock} state="default" index={2} />
      </ThemeProvider>
    );
